...
static struct {
    ...
    struct {
        ...
        ros::Timer PubTimer;
        // ros::Timer PubChassisReport;
        // ros::Timer PubWheelReport;
        // ros::Timer PubMpcTraj;
        // ros::Timer PubPlan;
        // ros::Timer PubEgoPose;
        // ros::Timer PubSysControllerRequest;
        // ros::Timer PubModuleControl;
    ...
    struct {
        struct {
            ...
            // added for control output
            // tRosIF_TpcSub<endpoint_msgs::ControlCommand> ControlCommand;
            tRosIF_TpcSub<lac_msgs::LacControlCmd> LacControlCmd;
        ...
        struct {
            ...
            // added for control input
            // tRosIF_TpcPub<endpoint_msgs::ChassisReport>              ChassisReport;
            // tRosIF_TpcPub<endpoint_msgs::WheelReport>                WheelReport;
            // tRosIF_TpcPub<planning_msgs::Planning>                   Plan;
            // tRosIF_TpcPub<mla_localization_msgs::MLALocalization>    EgoPose; 
            // tRosIF_TpcPub<system_manager_msgs::SysControllerRequest> SysControllerRequest;
            // tRosIF_TpcPub<framework_status_msgs::ModuleControl>      ModuleControl;
...
ros_HelloCM_CB_TpcIn(const hellocm_msgs::CM2Ext::ConstPtr &msg)
{
    ...
    auto in = &RosIF.Topics.Sub.CM2Ext.Msg;

    in->CarPosX_m   = msg->CarPosX_m;
    in->CarPosY_m   = msg->CarPosY_m;
    in->CarPosA_rad = msg->CarPosA_rad;
    in->CarYawRate_rads = msg->CarYawRate_rads;
    in->CarWhlAng_rad = msg->CarWhlAng_rad;
    in->CarWhlAngRate_rads = msg->CarWhlAngRate_rads;
    in->CarSpd_mps  = msg->CarSpd_mps;

    // in->chassis_report = msg->chassis_report;
    // in->wheel_report   = msg->wheel_report;
    // in->egopose        = msg->egopose;
    // in->plan           = msg->plan;
...
static void
ros_HelloCM_CB_TpcInControlCommand(const lac_msgs::LacControlCmd::ConstPtr &msg)
{
    RosIF.Topics.Pub.Ext2CM.Msg.control_command = *msg;
}
...
ros_HelloCM_CB_PubTimer(const ros::TimerEvent& Event) {
    ...
    out->Msg.TargetSpd_mps = RosIF.Topics.Sub.CM2Ext.Msg.CarSpd_mps;
    out->Msg.ControlCmd.ControlCmdEnable = RosIF.Topics.Sub.LacControlCmd.Msg.LacControlCmdEnable;
    out->Msg.ControlCmd.SteeringWhlRadReqCmd = RosIF.Topics.Sub.LacControlCmd.Msg.SteeringWhlRadReqCmd;
...
/*
static void
ros_HelloCM_CB_PubChassisReport(const ros::TimerEvent& Event) {

    ros::WallTime wtime;
    auto out = &RosIF.Topics.Pub.ChassisReport;
    out->Msg = RosIF.Topics.Sub.CM2Ext.Msg.chassis_report;
...
static void
ros_HelloCM_CB_PubWheelReport(const ros::TimerEvent& Event) {

    ros::WallTime wtime;
    auto out = &RosIF.Topics.Pub.WheelReport;
    out->Msg = RosIF.Topics.Sub.CM2Ext.Msg.wheel_report;
...
static void
ros_HelloCM_CB_PubPlan(const ros::TimerEvent& Event) {

    ros::WallTime wtime;
    auto out = &RosIF.Topics.Pub.Plan;
    out->Msg = RosIF.Topics.Sub.CM2Ext.Msg.plan;
...
static void
ros_HelloCM_CB_PubEgoPose(const ros::TimerEvent& Event) {
    
    ros::WallTime wtime;
    auto out = &RosIF.Topics.Pub.EgoPose;
    out->Msg = RosIF.Topics.Sub.CM2Ext.Msg.egopose;
...
static void
ros_HelloCM_CB_PubModuleControl(const ros::TimerEvent& Event) {
    
    ros::WallTime wtime;
    auto out = &RosIF.Topics.Pub.ModuleControl;
    out->Msg = RosIF.Topics.Sub.CM2Ext.Msg.module_control;
...
static void
ros_HelloCM_CB_PubSysControllerRequest(const ros::TimerEvent& Event) {

    ros::WallTime wtime;
    auto out = &RosIF.Topics.Pub.SysControllerRequest;
    out->Msg = RosIF.Topics.Sub.CM2Ext.Msg.sys_controller_request;
...
ros_HelloCM_Init(int Argc, char **Argv) {
    ...
    // LOG("  -> Publish /mla/egopose");
    // RosIF.Topics.Pub.EgoPose.Pub       = node->advertise<mla_localization_msgs::MLAAcceleration>("/mla/egopose", 10);
    // LOG("  -> Publish /vehicle/wheel_report");
    // RosIF.Topics.Pub.WheelReport.Pub   = node->advertise<endpoint_msgs::WheelReport>("/vehicle/wheel_report", 10);
    // LOG("  -> Publish /vehicle/chassis_report");
    // RosIF.Topics.Pub.ChassisReport.Pub = node->advertise<endpoint_msgs::ChassisReport>("/vehicle/chassis_report", 10);
    // LOG("  -> Publish /msd/planning/plan");
    // RosIF.Topics.Pub.Plan.Pub          = node->advertise<planning_msgs::Planning>("/msd/planning/plan", 10);
    // LOG("  -> Publish /system_manager/controller/request");
    // RosIF.Topics.Pub.SysControllerRequest.Pub = node->advertise<system_manager_msgs::SysControllerRequest>("/system_manager/controller/request", 10);
    // LOG("  -> Publish /msd/function_module_control");
    // RosIF.Topics.Pub.ModuleControl.Pub = node->advertise<framework_status_msgs::ModuleControl>("/msd/function_module_control", 10); 
    ...
    LOG("  -> Subscribe /msd/endpoint/control_command");
    //TODO
    RosIF.Topics.Sub.LacControlCmd.Sub = node->subscribe("/msd/endpoint/control_command", 1, ros_HelloCM_CB_TpcInControlCommand);


    /* Services */
    LOG("  -> Create Service '%s'", hellocm::srv_init_name.c_str());
    RosIF.Services.Init.Srv = node->advertiseService(hellocm::srv_init_name, ros_HelloCM_CB_SrvInit);
    ...
    if ((rv = node->hasParam(sbuf)) == true) {
        ...
        RosIF.CycleTime = 10;    // lei: the cycle of control is 20ms
    ...
    // RosIF.Timer.PubChassisReport        = node->createTimer(ros::Duration(20/1000.0),              ros_HelloCM_CB_PubChassisReport);
    // RosIF.Timer.PubWheelReport          = node->createTimer(ros::Duration(20/1000.0),              ros_HelloCM_CB_PubWheelReport);
    // RosIF.Timer.PubPlan                 = node->createTimer(ros::Duration(100/1000.0),             ros_HelloCM_CB_PubPlan);
    // RosIF.Timer.PubEgoPose              = node->createTimer(ros::Duration(20/1000.0),              ros_HelloCM_CB_PubEgoPose);
    // RosIF.Timer.PubModuleControl        = node->createTimer(ros::Duration(20/1000.0),              ros_HelloCM_CB_PubModuleControl);
    // RosIF.Timer.PubSysControllerRequest = node->createTimer(ros::Duration(20/1000.0),              ros_HelloCM_CB_PubSysControllerRequest);
